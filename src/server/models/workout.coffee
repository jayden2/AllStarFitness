...
module.exports = class Workout
	...
	@getSingleWorkout = (id, res) ->
		connection.acquire (err, con) ->
			con.query 'SELECT workouts.*, users.first_name, users.last_name, users.gender FROM workouts LEFT JOIN users on users.id = workouts.user_id WHERE workouts.id = ?', [id], (err, result) ->
				con.release()
				res.send result
				return
			return
