...
module.exports = class Exercise
	...
	@createExercise = (exercise, res) ->
		connection.acquire (err, con) ->
			con.query 'INSERT INTO exercises (title, description, image, duplicated, rep_time, def_set_start, def_set_end, def_rep_start, def_rep_end, def_tempo, date_created, favourite) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
			[exercise.title, exercise.description, exercise.image, exercise.duplicated, exercise.rep_time, exercise.def_set_start, exercise.def_set_end, exercise.def_rep_start, exercise.def_rep_end, exercise.def_tempo, exercise.date_created, exercise.favourite], (err, result) ->
	...
	@updateExercise = (exercise, id, res) ->
		connection.acquire (err, con) ->
			con.query 'UPDATE exercises SET title = ?, description = ?, image = ?, rep_time = ?, def_set_start = ?, def_set_end = ?, def_rep_start = ?, def_rep_end = ?, def_tempo = ?, favourite = ? WHERE id = ?',
			[exercise.title, exercise.description, exercise.image, exercise.rep_time, exercise.def_set_start, exercise.def_set_end, exercise.def_rep_start, exercise.def_rep_end, exercise.def_tempo, exercise.favourite, id], (err, result) ->
				con.release()
